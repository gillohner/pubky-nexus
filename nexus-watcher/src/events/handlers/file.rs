...
async fn ingest(
    ...
    match pubky_app_object {
        PubkyAppObject::Blob(blob) => {
            ingest_raw(user_id, file_id, &pubkyapp_file.content_type, blob.0, files_path).await
        }
...
///
/// Shared by the standard file handler (via `ingest`) and the universal file
/// handler (which fetches blob bytes directly without URI parsing).

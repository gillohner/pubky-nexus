...
pub async fn handle_put_event(event: &Event, moderation: Arc<Moderation>) -> Result<(), DynError> {
    ...
    match (pubky_object, resource) {
        ...
        (PubkyAppObject::Event(pubky_event), Resource::Event(event_id)) => {
            handlers::event::sync_put(pubky_event, user_id, event_id).await?
        }
...
pub async fn handle_del_event(event: &Event) -> Result<(), DynError> {
    ...
    match &event.parsed_uri.resource {
        ...
        Resource::Calendar(calendar_id) => {
            handlers::calendar::del(user_id, calendar_id.clone()).await?
        }
        Resource::Event(event_id) => handlers::event::del(user_id, event_id.clone()).await?,
        Resource::Attendee(attendee_id) => {
            handlers::attendee::del(user_id, attendee_id.clone()).await?
        }

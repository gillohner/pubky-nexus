...
tagged, can reply to any resource, and can share any resource. Only the app
...
The draft currently keeps `parent` post-only; this PoC deliberately extends the
same universal-reference rule to both `parent` and `embed` for evaluation.
...
```json
{
  ...
  "parent": "https://events.example/picnic",
...
- `parent` and new `embed` input are URI strings. They accept the same universal
  references: `parent` means the post is replying to that resource, while
  `embed` means the post is sharing/reposting it. The fields stay distinct even
  when both point to the same Resource. Collections still reject both fields.
- Legacy embed `{ "uri": "…", "kind": "link" }`
...
- References are limited to 1,024 characters. The draft's frozen whitespace
  ...
  are identifiers, not fetched locations. Pubky references require a valid key and
  a public `/pub/` path using the current parser; other `pubky*` schemes are reserved.

Nexus does not fetch or render external references. Acceptance is **not** a statement
...
| Endpoint | Change |
| --- | --- |
| `GET /v0/post/{author_id}/{post_id}/details` | Returns the exact `kind`, `content`, and `parent` / `embed` URI strings or `null`. |
...
| `GET /v0/resource/by-uri?uri=…` and `GET /v0/resource/{resource_id}/tags` | Also find parent/embed-only Resources, with an empty `tags` array when nobody has tagged them. URL-encode the lookup URI. |
| `GET /v0/stream/resources` and `/v0/stream/resources/ids` | Also include parent/embed-only Resources, including zero-score entries under `sorting=taggers_count`. |
...
The top-level `relationships.replied` and `relationships.reposted` fields retain
their existing Pubky-Post-only shape. Clients use `details.parent` and
`details.embed` for universal targets; this avoids pretending an external URL
has a Pubky author/post ID pair.

## Graph and cache behavior

A custom post is a normal `Post` node, not a generic Resource. A Pubky Post
parent/embed uses the existing `REPLIED` / `REPOSTED` relationship to a Post.
Other targets use the shared Resource node with distinct relationships:
`(post)-[:REPLIED {app: 'pubky.app'}]->(resource)` for a parent and
`(post)-[:EMBEDS {app: 'pubky.app'}]->(resource)` for an embed. `parent` is
therefore reply UI context; `embed` is share/repost UI content.
...
Changing or removing either field removes its old relationship. A Resource is
deleted only after its last relationship is gone: removing a tag or one post
reference must not delete a node that is still referenced elsewhere. Replays
preserve Resource creation time. Switching between external embeds and reposts
refreshes relationships and the affected posts' repost counts. Custom-kind edits
clear old mention edges.

Unfiltered Resource streams use Neo4j because the Redis Resource indexes only
track tags. `app=pubky.app` also matches REPLIED and EMBEDS references; tag-filtered queries
...
Previously rejected custom posts and previously discarded external references are
